...
public class Permissions {
    ...
    public static final int MY_PERMISSIONS_REQUEST_IMEINO = 4;
    public static Boolean checkPermissionGetImeiNo(Activity activity) {
        Boolean res = false;
        if (Build.VERSION.SDK_INT >= 23) {
            // Marshmallow+
            if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
                activity.requestPermissions(new String[]{Manifest.permission.READ_PHONE_STATE}, MY_PERMISSIONS_REQUEST_IMEINO);
            } else {
                res = true;
            }
        } else {
